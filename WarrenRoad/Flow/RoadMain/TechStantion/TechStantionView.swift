//
//  TechStantionView.swift

import SwiftUI

struct TechStantionView: View {
        ...
        GeometryReader { geometry in
            ZStack {
                ZStack {
                    ...
                    WaveShapeTechStation(offset: waveOffset)
                ...
                VStack(spacing: AdaptiveSize.spacing(20)) {
                    ...
                    if memory.equipmentStorage.isEmpty {
                        VStack(spacing: AdaptiveSize.spacing(24)) {
                            ...
                            VStack(spacing: AdaptiveSize.spacing(15)) {
                                ...
                                Text("Start organizing your office equipment by creating your first storage")
                        ...
                        ScrollView {
                            LazyVStack(spacing: AdaptiveSize.spacing(15)) {
                                ForEach(Array(memory.equipmentStorage.enumerated()), id: \.element.id) { index, storage in
                                    EquipmentStorageCardView(storage: storage)
                                        .onTapGesture {
                                            router.selectedTechIndex = index
                                            router.currentScreen = .addOrge                       }
                                }
                            }
...
// MARK: - Equipment Storage Card View
struct EquipmentStorageCardView: View {
    let storage: OfficeEquipmentStorage
    @State private var isAnimating = false
...
// MARK: - Add Equipment Storage View
struct AddEquipmentStorageView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .navigationTitle("New Equipment Storage")
...
// MARK: - Wave Shape
struct WaveShapeTechStation: Shape {
    var offset: CGFloat
