...
struct AddFurnitureView: View {
    ...
    @State private var selectedCategory = FurnitureCategory.chairs
    ...
    private var storage: OfficeFurnitureStorage? {
        guard storageIndex < memory.furnitureStorage.count else { return nil }
        return memory.furnitureStorage[storageIndex]
    }
    ...
    var body: some View {
        NavigationView {
            GeometryReader { geometry in
                ZStack {
                    // Warren Road themed background
                    ZStack {
                        // Base gradient with your colors
                        LinearGradient(
                        ...
                        WaveShapeFurniture(offset: waveOffset)
                    ...
                    VStack(spacing: AdaptiveSize.spacing(20)) {
                        VStack(spacing: AdaptiveSize.spacing(10)) {
                            Text(storage?.name ?? "Furniture Storage")
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: AdaptiveSize.cornerRadius(15))
                                .fill(Color.lightYellow.opacity(0.3))
                                .overlay(
                                    RoundedRectangle(cornerRadius: AdaptiveSize.cornerRadius(15))
                        ...
                        if storage?.items.isEmpty == true {
                            VStack(spacing: AdaptiveSize.spacing(20)) {
                                ...
                                Text("Add your first furniture item to get started")
                            ...
                            ScrollView {
                                LazyVStack(spacing: AdaptiveSize.spacing(12)) {
                                    ForEach(storage?.items ?? []) { item in
                                        FurnitureItemCardView(item: item)
                                    }
                                }
        ...
        .sheet(isPresented: $showingAddItem) {
            AddFurnitureItemView(storageIndex: storageIndex)
        }
    }
...
struct FurnitureItemCardView: View {
...
struct AddFurnitureItemView: View {
    ...
    var body: some View {
        NavigationView {
                    ...
                    ScrollView {
                        VStack(spacing: AdaptiveSize.spacing(20)) {
                                        ...
                                        HStack(spacing: AdaptiveSize.spacing(12)) {
                                            ForEach(FurnitureCategory.allCases, id: \.self) { category in
                                                Button(action: {
                                                    ...
                                                    .background(
                                                        RoundedRectangle(cornerRadius: AdaptiveSize.cornerRadius(12))
                                                            .fill(selectedCategory == category ?
                                                                  LinearGradient(
                                                                    gradient: Gradient(colors: [Color.green, Color.lightOlive.opacity(0.8)]),
                                                                    startPoint: .topLeading,
                                                                    endPoint: .bottomTrailing
                                                                  ) :
                                                                  LinearGradient(
                                                                    gradient: Gradient(colors: [Color.lightYellow.opacity(0.2), Color.lightYellow.opacity(0.1)]),
                                                                    startPoint: .topLeading,
                                                                    endPoint: .bottomTrailing
                                                                  )
                                                            )
                                                    ...
                                                    .overlay(
                                                        RoundedRectangle(cornerRadius: AdaptiveSize.cornerRadius(12))
                                                            .stroke(selectedCategory == category ? Color.green : Color.lightYellow.opacity(0.5), lineWidth: 2)
                                                    )
                                                }
                                            }
                                        }
                                    ...
                                    HStack(spacing: AdaptiveSize.spacing(8)) {
                                        ForEach(ItemCondition.allCases, id: \.self) { conditionOption in
                                            Button(action: {
                                                ...
                                                .background(
                                                    RoundedRectangle(cornerRadius: AdaptiveSize.cornerRadius(8))
                                                        .fill(condition == conditionOption ?
                                                              ...
                                                              LinearGradient(
                                                                gradient: Gradient(colors: [Color.lightYellow.opacity(0.3), Color.lightYellow.opacity(0.1)]),
                                                                startPoint: .topLeading,
                                                                endPoint: .bottomTrailing
                                                              )
                                                        )
                                                ...
                                                .overlay(
                                                    RoundedRectangle(cornerRadius: AdaptiveSize.cornerRadius(8))
                                                        .stroke(condition == conditionOption ? Color.green : Color.lightYellow.opacity(0.5), lineWidth: 1)
                                                )
                                            }
                                        }
                                    }
                            ...
                            Button(action: {
                                ...
                                if let storage = memory.furnitureStorage.first(where: { $0.id == memory.furnitureStorage[storageIndex].id }) {
                                    memory.addItemToFurnitureStorage(storageId: storage.id, item: newItem)
                                    memory.checkAndUnlockAchievements()
                                }
            ...
            .navigationTitle("Add Furniture Item")
...
struct FurnitureFloatingElement: View {
    let index: Int
...
// MARK: - Wave Shape
struct WaveShapeFurniture: Shape {
    var offset: CGFloat
...
#Preview {
    AddFurnitureView(storageIndex: 0)
}
